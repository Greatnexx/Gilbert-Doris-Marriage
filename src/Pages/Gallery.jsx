...
const Gallery = memo(() => {
  const [headerRef, headerVisible] = useScrollAnimations();
  const [gridRef, gridVisible] = useScrollAnimations({ threshold: 0.05 });

  const galleryItems = useMemo(() => [
    { image: "/assets/images/gallery1.jpg", priority: true },
    ...
    <>
      <section className="py-32 bg-gray-100">
        <div className="container mx-auto px-4">
          ...
          <div 
            ref={gridRef}
            className={`grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 transition-all duration-1000 transform ${
              gridVisible 
                ? 'opacity-100 translate-y-0' 
